...
def write_stats(gene_name,position,in_exon_exon,in_intron_exon_preds,predictions):
     ...
     totalAboveHalfPreds =sum(i >= 0.5 for i in predictions) 
     tp=round(sum(i >= 0.5 for i in in_exon_exon),4)
     ...
     print("SDAF")
     print(len(in_exon_exon),len(in_intron_exon_preds))
...
def calcDiff(gene_name,allVals):
    ...
    for position in ['first','middle','last']:
        ...
        in_intron_intron_preds= intron_preds[:]
        in_exon_exon=[]
        in_exon_intron=[]
        ...
        for exon in exons:
            ...
            ##in_exon_diff_preds=diffVals[(start-gene_start) : (end-gene_start)]
            in_exon_exon += in_exon_exon_preds
            for x in in_exon_exon_preds:
                in_intron_exon_preds.remove(x)
            in_exon_intron_preds=intron_preds[(start-gene_start) : (end-gene_start)]
            in_exon_intron += in_exon_intron_preds
            for x in in_exon_intron_preds:
                in_intron_intron_preds.remove(x)
        if position=='first':
            ...
            write_stats(gene_name,"first",in_exon_exon,in_intron_exon_preds,predictions) 
        if position=='middle':
            ...
            ##threshold=np.percentile(in_exon_intron_preds,99.9) #get 95th percentile
            write_stats(gene_name,"middle",in_exon_exon,in_intron_exon_preds,predictions2) 

            
            '''
            print("threshold",threshold)
            print("Length exon",len(in_exon_exon))
            print("Length labels",len(labels))
            tp=sum(i > threshold for i in in_exon_exon) 
            total_exons=len(in_exon_exon)
            '''
            '''
            #This is for the subtraction method
            totalAboveZeroPreds =sum(i > 0 for i in diffVals) 
            tp=aboveZeroPreds
            fp=totalAboveZeroPreds-tp
            fn=len(in_exon_exon)-tp
            tn=len(diffVals)-len(in_exon_exon)-fp
            print("Above Zero TP, FP,FN, TN", tp,fp,fn,tn)
            print("Sensitivity, Specificity, Accuracy", (tp / (tp +fn)),(tn/(tn+fp)), ((tp+tn)/(tp+tn+fp+fn)))
            '''

        if position=='last':
            exon_minus_intron['last']=diffVals
            predictions3= in_exon_exon+ in_intron_exon_preds
            write_stats(gene_name,"last",in_exon_exon,in_intron_exon_preds,predictions3) 
    ...
    '''
        print(position)
        print(len(exon_preds),len(diff_in_exons),len(in_intron_exon_preds),len(in_intron_intron_preds))
        print(mean(diff_in_exons), mean(in_intron_diff_preds))
        threshold=0.0
        print("Exon percent_above_threshold:",threshold,sum(i > threshold for i in diff_in_exons)/len(diff_in_exons))
        print("Intron percent_above_threshold:",threshold,sum(i > threshold for i in in_intron_diff_preds)/len(in_intron_diff_preds))
        threshold=0.3
        print("Exon percent_above_threshold:",threshold,sum(i > threshold for i in diff_in_exons)/len(diff_in_exons))
        print("Intron percent_above_threshold:",threshold,sum(i > threshold for i in in_intron_diff_preds)/len(in_intron_diff_preds))
        threshold=0.5
...
def calculate_stats(data):
  ...
  """
  if not data:
    return {"mean": None, "sd": None, "median": None}
...
def plotVals(args):
    ...
    difference_preds,middle_exon_preds,middle_intron_exon_preds = calcDiff(args.gene,allValues)
    ...
    print(len(difference_preds['middle']))
    print(len(middle_exon_preds),len(middle_intron_exon_preds))
    stats = calculate_stats(middle_exon_preds)
    print(f"{args.input} {gene}: {stats['mean']}±{stats['sd']} ({stats['median']})")
    stats = calculate_stats(middle_intron_exon_preds)
    print(f"{args.input} {gene}: {stats['mean']}±{stats['sd']} ({stats['median']})")
